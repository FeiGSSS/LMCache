...
from lmcache.v1.storage_backend.tiering.tier_manager import Tier
from lmcache.v1.storage_backend.tiering.tier_manager import TierManager
...
class FakeCPUBackend:
    ...
    def remove_if_evictable(self, key) -> bool:
        if key not in self.keys:
            return False
        memory_obj = self.objects.get(key)
        if memory_obj is not None and not memory_obj.can_evict:
            return False
...
def test_replace_promote_disk_does_not_evict_pinned_victim() -> None:
    hotness_policy = HotnessPolicy()
    cpu_backend = FakeCPUBackend(capacity_bytes=100, usage_bytes=95)
    disk_backend = FakeDiskBackend()
    manager = _make_tier_manager(hotness_policy, cpu_backend, disk_backend)

    victim_cpu_key = dumb_cache_engine_key(235)
    hot_disk_key = dumb_cache_engine_key(236)
    victim_obj = FakeMemoryObj(can_evict=False)
    promoted_obj = FakeMemoryObj()
    ...
    cpu_backend.keys.add(victim_cpu_key)
    cpu_backend.objects[victim_cpu_key] = victim_obj
    disk_backend.objects[victim_cpu_key] = FakeMemoryObj()
    disk_backend.objects[hot_disk_key] = promoted_obj

    assert not manager.replace_promote_key(hot_disk_key, victim_cpu_key)
    assert victim_cpu_key in cpu_backend.keys
    assert hot_disk_key not in cpu_backend.keys
    assert cpu_backend.removed_keys == []


def test_replace_promote_disk_keeps_victim_when_promoted_key_missing() -> None:
    hotness_policy = HotnessPolicy()
    cpu_backend = FakeCPUBackend(capacity_bytes=100, usage_bytes=95)
    disk_backend = FakeDiskBackend()
    manager = _make_tier_manager(hotness_policy, cpu_backend, disk_backend)

    victim_cpu_key = dumb_cache_engine_key(237)
    missing_disk_key = dumb_cache_engine_key(238)
    victim_obj = FakeMemoryObj()

    hotness_policy.observe_store(victim_cpu_key, prefix_pos=20, now=0.0)
    hotness_policy.observe_store(missing_disk_key, prefix_pos=0, now=0.0)
    hotness_policy.mark_resident(victim_cpu_key, Tier.CPU, True)
    hotness_policy.mark_resident(victim_cpu_key, Tier.DISK, True)
    hotness_policy.mark_resident(missing_disk_key, Tier.DISK, True)

    cpu_backend.keys.add(victim_cpu_key)
    cpu_backend.objects[victim_cpu_key] = victim_obj
    disk_backend.objects[victim_cpu_key] = FakeMemoryObj()

    assert not manager.replace_promote_key(missing_disk_key, victim_cpu_key)
    assert victim_cpu_key in cpu_backend.keys
    assert cpu_backend.removed_keys == []
    victim_state = hotness_policy.get_state(victim_cpu_key)
    missing_state = hotness_policy.get_state(missing_disk_key)
    assert victim_state is not None
    assert Tier.CPU in victim_state.resident_tiers
    assert missing_state is None or Tier.DISK not in missing_state.resident_tiers


def test_run_once_above_high_only_runs_replace_promotion() -> None:
    hotness_policy = HotnessPolicy()
    cpu_backend = FakeCPUBackend(capacity_bytes=100, usage_bytes=95)
    disk_backend = FakeDiskBackend()
    manager = _make_tier_manager(hotness_policy, cpu_backend, disk_backend)
    ...
    assert victim_cpu_key not in cpu_backend.keys
    assert hot_disk_key in cpu_backend.keys
    assert cpu_backend.get_usage_bytes() == 95
