...
DEFAULT_DATASETS_DIR = Path("/home/fei/research/datasets")
DEFAULT_PROMPTS_DIR = Path(__file__).parent / "lib_prompts"
DATASET_PROMPT_FILES = {
    "gsm8k": "gsm8k_prompt_original.txt",
}
...
def _load_local_data(name: str, split: str, datasets_dir: Path = DEFAULT_DATASETS_DIR) -> List[Dict[str, Any]]:
    """Generic local dataset loader supporting parquet and jsonl."""
    # Try full name and base name
    candidates = [datasets_dir / name, datasets_dir / name.split("/")[-1]]
...
def accuracy_eval(args: argparse.Namespace) -> None:
    ...
    prompts_dir = Path(args.prompt_dir) if args.prompt_dir else DEFAULT_PROMPTS_DIR

    split = DATASET_SPLITS.get(args.dataset, "test")
    data = _load_local_data(args.dataset, split, datasets_dir)

    prompt_prefix = ""
    if not args.zero_shot:
        prompt_filename = DATASET_PROMPT_FILES.get(args.dataset)
...
def main():
    ...
    parser.add_argument("--prompt-dir", default=str(DEFAULT_PROMPTS_DIR),
                        help=f"Path to prompt files directory (default: {DEFAULT_PROMPTS_DIR})")
